...
public class PublicKey: Key {
    ...
    public func pemString() throws -> String {
        ...
        let pem = SwiftyEC.format(keyData: data, withPemType: "EC PUBLIC KEY")
        return pem
    }
    
    /// Creates a public key with a keychain key reference.
    /// This initializer will throw if the provided key reference is not a public EC key.
    ///
    /// - Parameter reference: Reference to the key within the keychain.
    /// - Throws: SwiftyECError
    public required init(reference: SecKey) throws {
        
        guard SwiftyEC.isValidKeyReference(reference, forClass: kSecAttrKeyClassPublic) else {
            throw SwiftyECError.notAPublicKey
        }
    ...
    /// This method throws if SwiftyEC cannot extract data from the key.
    ///
    ...
    required public init(data: Data) throws {
        ...
        reference = try SwiftyEC.addKey(dataWithoutHeader, isPublic: true, tag: tag)
    }
