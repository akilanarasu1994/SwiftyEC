...
public class ClearMessage: Message {
    ...
    public convenience init(string: String, using encoding: String.Encoding) throws {
        guard let data = string.data(using: encoding) else {
            throw SwiftyECError.stringToDataConversionFailed
        }
        self.init(data: data)
    }
    ...
    public func string(encoding: String.Encoding) throws -> String {
        guard let str = String(data: data, encoding: encoding) else {
            throw SwiftyECError.dataToStringConversionFailed
        }
        return str
    }
    ...
    public func encrypted(with key: PublicKey, padding: Padding) throws -> EncryptedMessage {
        ...
        while idx < decryptedDataAsArray.count {
            ...
            guard status == noErr else {
                throw SwiftyECError.chunkEncryptFailed(index: idx)
            }
    ...
    public func signed(with key: PrivateKey, digestType: Signature.DigestType) throws -> Signature {
        ...
        guard digest.count <= maxChunkSize else {
            throw SwiftyECError.invalidDigestSize(digestSize: digest.count, maxChunkSize: maxChunkSize)
        }
        ...
        guard status == noErr else {
            throw SwiftyECError.signatureCreateFailed(status: status)
        }
    ...
    public func verify(with key: PublicKey, signature: Signature, digestType: Signature.DigestType) throws -> Bool {
        ...
        if status == errSecSuccess {
            ...
            throw SwiftyECError.signatureVerifyFailed(status: status)
        }
    }
    ...
    func digest(digestType: Signature.DigestType) -> Data {
        ...
        switch digestType {
        case .sha1:
            digest = (data as NSData).swiftyECSHA1()
        case .sha224:
            digest = (data as NSData).swiftyECSHA224()
        case .sha256:
            digest = (data as NSData).swiftyECSHA256()
        case .sha384:
            digest = (data as NSData).swiftyECSHA384()
        case .sha512:
            digest = (data as NSData).swiftyECSHA512()
        }
