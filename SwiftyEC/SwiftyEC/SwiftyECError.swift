...
public enum SwiftyECError: Error {
    ...
    var localizedDescription: String {
        ...
        case .invalidDigestSize(let digestSize, let maxChunkSize):
            return "Provided digest type produces a size (\(digestSize)) that is bigger than the maximum chunk size \(maxChunkSize) of the EC key"
        ...
        case .notAPublicKey:
            return "Provided key is not a valid EC public key"
        case .notAPrivateKey:
            return "Provided key is not a valid EC pivate key"
        }
    }
}
