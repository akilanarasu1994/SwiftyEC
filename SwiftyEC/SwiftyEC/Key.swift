...
//  SwiftyEC
//
//  Created by Akilan Arasu on 06/26/17.
//  Copyright © 2017 AkilanArasu. All rights reserved.
//
...
public extension Key {
    ...
    func data() throws -> Data {
        return try SwiftyEC.data(forKeyReference: reference)
    }
    ...
    /// - Throws: SwiftyECError
    init(base64Encoded base64String: String) throws {
        print("SwiftEC: Base64String key: ", base64String)
    ...
    /// - Throws: SwiftyECError
    init(pemEncoded pemString: String) throws {
        let base64String = try SwiftyEC.base64String(pemEncoded: pemString)
    ...
    init(pemNamed pemName: String, in bundle: Bundle = Bundle.main) throws {
        guard let path = bundle.path(forResource: pemName, ofType: "pem") else {
            throw SwiftyECError.pemFileNotFound(name: pemName)
        }
    ...
    init(derNamed derName: String, in bundle: Bundle = Bundle.main) throws {
        guard let path = bundle.path(forResource: derName, ofType: "der") else {
            throw SwiftyECError.derFileNotFound(name: derName)
        }
